...
The SDK is part of the West manifest in NCS and is automatically downloaded when running `west update`. By default, it is downloaded to `<nRF Connect SDK path>/modules/memfault-firmware-sdk/`.
...
You can find more details on the available options using `menuconfig`, `guiconfig`, and in the Kconfig sources in [`modules/memfault-firmware-sdk/ports/zephyr/Kconfig`](https://github.com/memfault/memfault-firmware-sdk/blob/master/ports/zephyr/Kconfig).
