...
Any code using the `StructuredLog` API from the `structured-log-lib`
library (now removed) must be changed to call the equivalent methods in the
`CustomEvent` API from the `custom-event-lib` library, as described below.
...
Custom Events appear on the Device Timeline, under "Custom Events". For each
...
- Defining the log types and data payloads your applications and services need
  to log
- Add the `custom-event-lib` library to the component from which you want to log
...
### Add the `custom-event-lib` library

The `custom-event-lib` library is a small Java library that wraps the Binder
...
#### Add `custom-event-lib` to AOSP-built Java apps

Make sure the
[Bort SDK has been integrated](https://mflt.io/android-getting-started) with
your AOSP build first.

Once integrated, a target called `memfault-custom-event-lib` will be available.
...
LOCAL_STATIC_JAVA_LIBRARIES := \
    memfault-custom-event-lib \
```

Or if you are using an Android.bp file, add `memfault-custom-event-lib` to a
...
#### Add `custom-event-lib` to Gradle-built, Java or Kotlin apps

For apps that are build outside of the AOSP build system using Gradle, the
easiest way to include `custom-event-lib` in your project is using Gradle's
`includeBuild` API.
...
includeBuild('/path/to/bort/MemfaultPackages') {
    dependencySubstitution {
        substitute module('com.memfault:custom-event-lib') with project(':custom-event-lib')
    }
}
```
...
```
dependencies {
    implementation 'com.memfault:custom-event-lib'
    ...
}
```

When you build your project, the `custom-event-lib` library will be built as
