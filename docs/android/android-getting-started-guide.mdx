...
We also recommend updating the other properties as necessary, for
...
Note that **updating the target SDK version may be required to pass
[CTS](https://source.android.com/compatibility/cts)**. This can be
done by changing the `TARGET_SDK_VERSION` property in
