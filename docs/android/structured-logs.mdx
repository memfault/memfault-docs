...
interest for debugging purposes. Your application code can leverage this to log
crucial information and get it surfaced in the Memfault UI. The Bort SDK itself
also provides built-in sources that log events of interest from common Android
...
represented with a dot. Clicking or hovering over it will reveal the logged JSON
data:
...
Binder service that log producers can use to send Structured Logs to the daemon.
...
when the buffer has reached a certain size limit or when requested explicitely
...
easiest way to include `structured-log-lib` in your project is using Gradle's
`includeBuild` API.
...
When you build your project, the `structured-log-lib` library will be built as
